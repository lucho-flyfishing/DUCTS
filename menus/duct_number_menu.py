...
def duct_number_menu(W, go_back, go_next):
    ...
    main_branch_lbl = Label(middle_frame, text='Número del ramal que va a usar como ducto principal:',
                            font=('Arial', 26), bg='gray5', fg='OrangeRed2')
    ...
    def save_duct_number():
        value = duct_number_entry.get() 

        if value == duct_number_placeholder:
            app_state.duct_number.set(0) 
        else:
            try:
                app_state.duct_number.set(int(value))
            except ValueError:
                print("Error: Ingrese un número válido")

        print(f'El número de ductos es: {app_state.duct_number.get()}')
    ...
    def save_main_branch():
        ...
        if value == 'Escribe aquí...':
            ...
            try:
                ...
                print("Error: Ingrese un número válido")
        print(f'El ducto principal es el número: {app_state.main_branch.get()}')
