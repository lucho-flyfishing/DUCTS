...
def bilinear(x, y, x0, x1, y0, y1, q11, q12, q21, q22):
    ...
    q11/q12 comparten fila x0 y varían en y (theta); q21/q22 son la fila x1.
    Primero se interpola en y dentro de cada fila, luego en x entre filas."""
    r1 = interp(y, y0, y1, q11, q12)
    r2 = interp(y, y0, y1, q21, q22)
    return interp(x, x0, x1, r1, r2)
...
# NOTE: the ASHRAE 4-2 source table prints its header as "10 15 30 30 45..."
# — the duplicated 30 is a typo in the source; the true axis is 10, 15, 20, 30
# (confirmed by fitting 4-3, which reuses the same data with a clean header).
theta_values = [10, 15, 20, 30, 45, 60, 90, 120, 150, 180]

Co_table = {
    0.06: [0.26, 0.27, 0.40, 0.56, 0.71, 0.86, 1.00, 0.99, 0.98, 0.98],
    0.10: [0.24, 0.26, 0.36, 0.53, 0.69, 0.82, 0.93, 0.93, 0.92, 0.91],
    0.25: [0.17, 0.19, 0.22, 0.42, 0.60, 0.68, 0.70, 0.69, 0.67, 0.66],
    0.50: [0.14, 0.13, 0.15, 0.24, 0.35, 0.37, 0.38, 0.37, 0.36, 0.35],
    1.00: [0.0] * 10,
    2.00: [0.23, 0.20, 0.20, 0.20, 0.24, 0.28, 0.54, 0.78, 1.00, 1.10],
    4.00: [0.81, 0.64, 0.64, 0.64, 0.88, 1.10, 2.80, 4.40, 5.70, 6.60],
    6.00: [1.80, 1.40, 1.40, 1.40, 2.00, 2.50, 6.60, 10.00, 13.00, 15.00],
    10.0: [5.00, 5.00, 5.00, 5.00, 6.50, 8.00, 19.0, 29.0, 37.0, 43.0],
}
